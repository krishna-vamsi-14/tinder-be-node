...
- use validator.js
- For email validation, password validation, isURL valiation, isEmail, isStrongPassword


Episode 9 : 

- Never trust req.body.
- Lets sanitize signup api. 
- Hasing password. 
- bcrypt.hash(password, saltRounds);
- Add login route by using bcrypt.compare(password, password saved in db)
...
1. If user wants to connect with server -> There will be a tcp ip protocol. 
2. When we login -> Server generates a JWT token -> User will store the JWT token. 
3. After that every time JWT token will be sent with every request. 
4. In every request, server will validate if that token is valid or not.
5. Cookies ?? -> When we logging in with login api, server will send a successful response, along with it, it will send a cookie which has token. 
6.Browser will store the cookie and whenever we are api call, cookie will be sent to server, and then that cookie will be validated.
7.Cookie will travel/sent to server for each and every api. 

8. When will the cookie won't work ?? 
9. If we gave expiry time to cookie or jwt token, then cookie won't work after cookie time.
10. Whenever user is logging in, server will create a jwt token, attach to a cookie, and sent it to the browser. 


11. Express gives a way to send a cookie in response. 
res.cookie


12. We need a cookieparser to read the cookies. 
npm i cookie-parser
const cookieParser = require("cookie-parser");

app.use(cookieParser())

13. JWT token will consists of three types - Header, payload, signature. 
you can check these will be in red, pink, blue colours in jwt office documentation. 

Install jsonwebtoken npm package for generating json web token. 
It will give a way to signing the jwt token and verifying the token. 

jwt.sign
jwt.verify


- Create a auth middleware, and add auth middleware in all the business logic apis. 
- Set the expires in cookies and expiryIn in jwt token. 


Mongoose schema methods : 

- User schema defines the schema of the user. We can attach methods to the schema, that will be applicable to all the users. 
- These are the helper methods. 

- We can offload creation of jwt creation logic from handler to schema method.
- Add create jwt token schema method in user schema.
- Add validate password method to user schema.


Episode 11 : 

- Express router. ( We can check express router documentation )
...
Both works in similar for us. Almost no difference, express manages internally.


Episode 12 : 

Indexes in database - Because, queries will become slow because of large database,
If we put indexes on some columns querying will be faster than usual. 
Mongodb automatically adds an index if you say your column value is unique.
For example email in user schema is unique and mongodb automatically add index on email. 


$or and $and queries in mongodb.
-sparse index.
-unique index.
-index

-compound index ( If we want to create index with two column values)
-For us we need to create compound index for fromUserId and toUserId in connectionRequest Schema.
- connectionRequest.index({fromUserId: 1, toUserId: 1})

- Creating index unnessiraly also comes with a cost. 
- Be specific about indexes. 
- Initally there is no need of creating index for less users.